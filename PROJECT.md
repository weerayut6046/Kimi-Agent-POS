...
`shift_readings` ไม่ได้เก็บชนิดน้ำมันของตัวเอง แต่อ้างผ่าน `nozzles.product_id` การแก้ชนิดน้ำมันของหัวจ่ายที่มีประวัติแล้วจะทำให้ยอดลิตรและกระทบยอดถังของกะเก่าย้ายไปผิดชนิดย้อนหลัง migration จึงแยกสองทาง:

| สถานะหัวจ่าย GSH91 | การทำงาน                                                                                         |
| ------------------ | ------------------------------------------------------------------------------------------------ |
| ยังไม่มีประวัติกะ  | เปลี่ยนเป็น GSH95 ในที่เดิม แล้วลบถัง/สินค้า GSH91 ที่ไม่มีประวัติทิ้ง                           |
| มีประวัติกะแล้ว    | ปลดระวางหัวเดิม (`active = false`) และเพิ่มหัว GSH95 ใหม่บนตู้เดิม โดยยกเลขมิเตอร์และเลข P มาต่อ |

ถังและสินค้า GSH91 จะถูกลบเฉพาะเมื่อไม่เหลือประวัติอ้างอิงแล้วเท่านั้น เพราะ `tank_refills`/`tank_readings` เป็น `ON DELETE CASCADE` และ `sale_items`/`price_changes` เป็น `ON DELETE SET NULL` ถ้ายังมีประวัติ ระบบจะเก็บถังไว้และปิดการใช้งานสินค้าแทน

**สิ่งที่ต้องทำต่อด้วยมือ** เมื่อปั๊มเคยขาย GSH91 จริง: ถัง GSH91 จะยังค้างอยู่ในรายการถังพร้อมยอดคงเหลือเดิม ให้ผู้ดูแลปรับยอดผ่านหน้ากระทบยอดถัง (บันทึกค่าวัดจริง) หรือลบถังทิ้งจากหน้าตั้งค่าเมื่อยืนยันแล้วว่าไม่ต้องใช้ประวัติรับเข้าของถังนั้น
