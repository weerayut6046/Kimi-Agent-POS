...
ผังของปั๊มคือ 2 ตู้จ่าย ตู้ละ 2 หัว จ่ายแก๊สโซฮอล์ 95 และดีเซลทั้งสองตู้ ฐานข้อมูลที่สร้างจาก seed รุ่นก่อนหน้าจะตั้งให้ตู้ 2 จ่าย GSH91 migration นี้ปรับให้ตรงผังจริงโดยรันซ้ำได้และแยกทำงานตามสาขา

ปั๊มไม่เคยขาย GSH91 เลย หัวจ่ายที่ฐานข้อมูลบันทึกว่าเป็น GSH91 จึงเป็นหัวจ่าย GSH95 ที่ถูกตั้งชนิดน้ำมันผิดมาตั้งแต่ต้น migration จึงย้ายหัวจ่ายเป็น GSH95 **ในที่เดิม** ไม่สร้างหัวใหม่และไม่ปลดระวางหัวเก่า

`shift_readings` ไม่ได้เก็บชนิดน้ำมันของตัวเอง แต่อ้างผ่าน `nozzles.product_id` การย้ายในที่เดิมจึงทำให้ประวัติกะที่เคยถูกนับเป็น GSH91 กลับไปรายงานเป็น GSH95 และยอดลิตรไปกระทบยอดถัง GSH95 ตามที่เกิดขึ้นจริง

สิ่งที่ migration ไม่แตะต้อง: `shift_readings.price_per_liter` เป็น snapshot ราคาที่ใช้ปิดกะและกระทบกับเงินสดที่นับจริงไปแล้ว การเขียนทับจะทำให้ยอดกะที่ปิดแล้วไม่ตรงกับเงินที่รับมาจริง จึงเก็บไว้ตามที่บันทึกไว้เดิม

ถังและสินค้า GSH91 จะถูกลบเฉพาะเมื่อไม่เหลือประวัติอ้างอิงแล้วเท่านั้น เพราะ `tank_refills`/`tank_readings` เป็น `ON DELETE CASCADE` และ `sale_items`/`price_changes`/`stock_count_items` เป็น `ON DELETE SET NULL` ถ้ายังมีประวัติ ระบบจะเก็บถังไว้และปิดการใช้งานสินค้าแทน พร้อมขึ้น `NOTICE` ให้ผู้ดูแลตรวจสอบเอง
