...
> อัปเดตล่าสุด: 21 กรกฎาคม 2026 — source/build ในเครื่อง `2.0.0`; รุ่นเผยแพร่บน GCS `2.0.0`; เว็บออนไลน์ deploy ที่ Vercel + Railway (ดูหัวข้อ 11)

## 1. สถานะปัจจุบัน

| รายการ            | สถานะ                                                               |
| ----------------- | ------------------------------------------------------------------- |
| เวอร์ชันใน source | `2.0.0`                                                             |
| รุ่นเผยแพร่ล่าสุด | `2.0.0`                                                             |
| Branch หลัก       | `main`                                                              |
...
| การทำงานออฟไลน์   | ต้องเชื่อมอินเทอร์เน็ตเพื่อใช้ฐานข้อมูลกลาง                          |

ไฟล์ติดตั้งรุ่นล่าสุดที่เผยแพร่:

- [POS-Pump-Setup-2.0.0.exe](https://storage.googleapis.com/kimi-agent-pos-updates/POS-Pump-Setup-2.0.0.exe)
- [POS-Pump-Portable-2.0.0.exe](https://storage.googleapis.com/kimi-agent-pos-updates/POS-Pump-Portable-2.0.0.exe)
- [latest.yml](https://storage.googleapis.com/kimi-agent-pos-updates/latest.yml)

รุ่น `1.0.21`–`1.0.24` เพิ่ม Wizard ภาษาไทย, workforce, มิเตอร์ประวัติกะ, แก้ responsive UI และเริ่ม sign executable ด้วย self-signed certificate (`CN=PumpPOS Code Signing`) รุ่น `2.0.0` (เผยแพร่ 21 กรกฎาคม 2026) ย้ายข้อมูลจาก SQLite ขึ้น Supabase PostgreSQL ใน private schema `pos`, เปลี่ยน Desktop เป็น shell ที่โหลดระบบกลาง, ใช้ signed staff session, ให้ Railway เชื่อมผ่าน session pooler และแยก tRPC request เพื่อแก้หน้า Dashboard โหลดค้าง การสำรอง/กู้คืนย้ายไปจัดการจาก Supabase Dashboard — installer + Portable + blockmap + `latest.yml` เผยแพร่ผ่าน GCS
...
- สำรอง/กู้คืนฐานข้อมูลผ่าน Supabase Dashboard และ Point-in-Time Recovery ตามแผนบริการ
...
ในโหมด Desktop, Electron เปิดเว็บ production ใน `BrowserWindow` และคง IPC สำหรับพิมพ์ใบเสร็จ จึงไม่มีฐานข้อมูลหรือ database secret อยู่ในเครื่องลูกข่าย
...
- Desktop โหลดระบบกลางจากเว็บที่ deploy แล้ว จึงไม่เก็บ database secret หรือ `APP_SECRET` ไว้ในเครื่องลูกข่าย
...
- การสำรอง/กู้คืนและ Point-in-Time Recovery จัดการจาก Supabase Dashboard
...
ตัวแปรแวดล้อมบน Railway: `APP_ID`, `APP_SECRET` (สุ่มตอน deploy), `DATABASE_URL` (Supabase session pooler สำหรับ backend แบบ long-lived), `DATABASE_POOL_SIZE=5`, `SEED_ON_START=false`
