...
// ============ กลุ่มสิทธิ์เมนู ============
export const staffAccessGroups = posSchema
  .table("staff_access_groups", {
    id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
// ============ พนักงาน ============
export const staffUsers = posSchema
  .table(
    "staff_users",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
export const staffBranches = posSchema
  .table(
    "staff_branches",
    {
...
export const workShiftTemplates = posSchema
  .table(
...
export const workSchedules = posSchema
  .table(
    "work_schedules",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
export const employeeProfiles = posSchema
  .table(
    ...
    t => ({
      staffUnique: uniqueIndex("employeeprofile_staff_unique").on(t.staffId),
    })
  )
  .enableRLS();

export const payrollRecords = posSchema
  .table(
    "payroll_records",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
      ...
      overtimeAmount: numeric("overtime_amount", {
        ...
        .notNull()
        .default(0),
      bonus: numeric("bonus", { precision: 18, scale: 3, mode: "number" })
        .notNull()
        .default(0),
...
export const products = posSchema
  .table(
    "products",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
      ...
      cost: numeric("cost", { precision: 18, scale: 3, mode: "number" })
        .notNull()
        .default(0),
      stockQty: numeric("stock_qty", {
        ...
        .notNull()
        .default(0),
      lowStockAt: numeric("low_stock_at", {
        precision: 18,
        scale: 3,
        mode: "number",
      })
...
export const nozzles = posSchema
  .table(
    "nozzles",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
      ...
      tankId: integer("tank_id").references((): AnyPgColumn => fuelTanks.id, {
        onDelete: "set null",
      }),
      ...
      currentMeter: numeric("current_meter", {
        ...
        .notNull()
        .default(0),
      currentMoney: numeric("current_money", {
        precision: 18,
        scale: 3,
        mode: "number",
      })
        .notNull()
        .default(0), // มิเตอร์เงินสะสม P (บาท)
      active: boolean("active").notNull().default(true),
    },
    t => ({
...
export const shiftReadings = posSchema
  .table(
    "shift_readings",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
      ...
      openMeter: numeric("open_meter", {
        precision: 18,
        scale: 3,
        mode: "number",
      }).notNull(),
      closeMeter: numeric("close_meter", {
        precision: 18,
        scale: 3,
        mode: "number",
      }),
      openMoney: numeric("open_money", {
        precision: 18,
        scale: 3,
        mode: "number",
      })
        .notNull()
        .default(0), // P ตั้งต้น (บาท)
      closeMoney: numeric("close_money", {
        precision: 18,
        scale: 3,
        mode: "number",
      }), // P ปิดกะ (บาท)
...
export const sales = posSchema
  .table(
    "sales",
    {
      ...
      discount: numeric("discount", { precision: 18, scale: 3, mode: "number" })
        .notNull()
        .default(0),
      vatRate: numeric("vat_rate", { precision: 18, scale: 3, mode: "number" })
        .notNull()
        .default(7),
      ...
      paymentMethod: text("payment_method", {
        ...
        .notNull()
        .default("cash"),
      received: numeric("received", { precision: 18, scale: 3, mode: "number" })
        .notNull()
        .default(0),
      changeAmt: numeric("change_amt", {
        precision: 18,
        scale: 3,
        mode: "number",
      })
...
export const saleItems = posSchema
  .table(
    "sale_items",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
export const rewards = posSchema
  .table(
    "rewards",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
export const rewardRedemptions = posSchema
  .table(
    "reward_redemptions",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
// ============ ถังน้ำมัน ============
export const fuelTanks = posSchema
  .table(
    "fuel_tanks",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
      ...
      currentLiters: numeric("current_liters", {
        precision: 18,
        scale: 3,
        mode: "number",
      })
        .notNull()
        .default(0),
      lowAlertAt: numeric("low_alert_at", {
        precision: 18,
        scale: 3,
        mode: "number",
      })
...
export const tankRefills = posSchema
  .table(
    "tank_refills",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
export const tankReadings = posSchema
  .table(
    "tank_readings",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
export const customers = posSchema
  .table("customers", {
    id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
    name: text("name").notNull(),
    taxId: text("tax_id").notNull().default(""),
    ...
    creditLimit: numeric("credit_limit", {
      ...
      .notNull()
      .default(0), // วงเงินเครดิต (0 = ไม่จำกัด)
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
  })
...
// ============ ใบกำกับภาษีเต็มรูป ============
export const taxInvoices = posSchema
  .table(
    "tax_invoices",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
export const debtPayments = posSchema
  .table(
    "debt_payments",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
      ...
      method: text("method", { enum: ["cash", "qr", "transfer"] })
        .notNull()
        .default("cash"),
      shiftId: integer("shift_id").references(() => shifts.id, {
        onDelete: "set null",
      }), // กะที่เปิดอยู่ตอนรับชำระ (ถ้ามี)
      staffName: text("staff_name").notNull().default(""),
      note: text("note"),
      createdAt: timestamp("created_at", { withTimezone: true })
        .notNull()
...
// ============ ค่าใช้จ่ายหน้าร้าน ============
export const expenses = posSchema
  .table(
    "expenses",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
export const priceChanges = posSchema
  .table(
    "price_changes",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
      ...
      productId: integer("product_id").references(() => products.id, {
        onDelete: "set null",
      }), // nullable — เก็บประวัติไว้แม้ลบสินค้า
...
// ============ Audit log ============
export const auditLogs = posSchema
  .table(
    "audit_logs",
    {
      id: integer("id").primaryKey().generatedByDefaultAsIdentity(),
...
export const assistantSettings = posSchema
  .table(
    "assistant_settings",
    {
      branchId: integer("branch_id")
      ...
      provider: text("provider", { enum: ["ollama", "deepseek"] })
        .notNull()
        .default("ollama"),
      ollamaModel: text("ollama_model").notNull().default("qwen3:4b-instruct"),
...
export const assistantActionProposals = posSchema
  .table(
    ...
    t => ({
      ...
      expiresIdx: index("assistant_action_proposal_expires_idx").on(
        t.expiresAt
      ),
    })
  )
