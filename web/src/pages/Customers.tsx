...
export default function Customers() {
  ...
  const { data: rows, isLoading } = trpc.customers.list.useQuery({ q: search || undefined });
  ...
  const submit = () => {
    ...
    const payload = {
      ...
      branch: edit.branchType === "hq" ? "สำนักงานใหญ่" : `สาขาที่ ${edit.branchNo.trim()}`,
    ...
    <div className="space-y-5">
              ...
              {(rows ?? []).map((c) => (
                <TableRow key={c.id}>
                  <TableCell>
                    ...
                    {c.address && <div className="text-xs text-muted-foreground whitespace-pre-line">{c.address}</div>}
                  </TableCell>
                  <TableCell className="font-mono text-xs">{c.taxId || "-"}</TableCell>
                  <TableCell className="text-sm">{c.branch || "-"}</TableCell>
                  <TableCell className="text-sm">{c.phone || "-"}</TableCell>
                  <TableCell className="text-sm">{c.vehiclePlate || "-"}</TableCell>
                  {canManage && (
                    <TableCell>
                      <div className="flex gap-1">
                        <Button size="icon" variant="ghost" className="h-8 w-8" title="แก้ไข"
                          onClick={() => { setErr(""); setEdit(formFromCustomer(c)); }}>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (confirm(`ยืนยันลบลูกค้า "${c.name}"?`)) remove.mutate({ id: c.id });
                          }}
                        >
      ...
      <Dialog open={!!edit} onOpenChange={(o) => !o && setEdit(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="font-heading">{edit?.id ? "แก้ไขลูกค้า" : "เพิ่มลูกค้า"}</DialogTitle>
          </DialogHeader>
          {edit && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input inputMode="numeric" maxLength={13} value={edit.taxId} onChange={(e) => setEdit({ ...edit, taxId: e.target.value })} />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-4">
                  <RadioGroup
                    ...
                    onValueChange={(v) => setEdit({ ...edit, branchType: v as "hq" | "branch" })}
                  >
                    <div className="flex items-center gap-1.5">
                      <RadioGroupItem value="hq" id="c-hq" />
                      <Label htmlFor="c-hq" className="font-normal">สำนักงานใหญ่</Label>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <RadioGroupItem value="branch" id="c-branch" />
                      <Label htmlFor="c-branch" className="font-normal">สาขาที่</Label>
                    </div>
                  </RadioGroup>
                  {edit.branchType === "branch" && (
                    <Input className="w-28" placeholder="เช่น 00078" value={edit.branchNo} onChange={(e) => setEdit({ ...edit, branchNo: e.target.value })} />
                  )}
                </div>
              </div>
              <div className="space-y-1.5">
          ...
          <DialogFooter>
            <Button
              className="w-full"
              disabled={!edit?.name.trim() || create.isPending || update.isPending}
              onClick={submit}
            >
              บันทึก
            </Button>
