...
### อ่านมิเตอร์ตัดกะจากรูปภาพ

- หน้าจัดการกะรองรับการเลือก JPG/PNG/WebP หลายภาพหรือถ่ายเพิ่มทีละภาพ ระบบสร้างสำเนาที่ลบ EXIF/GPS และแนะนำให้ถ่าย 3 มุมเพื่อให้เงาสะท้อนเคลื่อนตำแหน่ง
- Computer vision แบบ deterministic อ่าน seven-segment ในเครื่องด้วย threshold หลาย channel พร้อมวัดแสงสะท้อนและโหวตค่าที่ตรงกันอย่างน้อย 2 ใน 3 หากหา LCD ไม่พบหรือจอเล็กจะวิเคราะห์ซ้ำด้วยความละเอียดสูง จากนั้นสร้างแผ่นภาพที่มีภาพเต็มและภาพขยายจอซ้าย/ขวาให้ Gemini Vision อ่านแบบอิสระโดยไม่เห็นผล local OCR
- ค่าจะผ่านอัตโนมัติเฉพาะเมื่อ local OCR/consensus และ AI confidence สูงอ่านตัวเลขกับโหมด L/P ตรงกัน หาก AI ไม่พร้อม ไม่ชัด หรืออ่านต่างกัน ระบบบังคับให้พนักงานตรวจและยืนยันเอง
- หน้าตรวจผลแสดงระดับคุณภาพของแต่ละภาพและเตือนเมื่อจอเล็ก ภาพเอียง เบลอ มืด หรือมีแสงสะท้อน ภาพที่จัดว่าคุณภาพต่ำจะไม่ผ่านอัตโนมัติแม้ OCR และ AI อ่านตรงกัน
- ภาพอยู่ในหน่วยความจำของหน้าเว็บชั่วคราวและไม่บันทึกลงฐานข้อมูล; สำเนา JPEG ถูกส่งไป Gemini เฉพาะเมื่อกดอ่านและตรวจ โดย request กำหนด `store: false`
- ฝั่ง server ใช้ `GEMINI_API_KEY` และเลือกโมเดลด้วย `GEMINI_METER_VERIFY_MODEL` (ค่าเริ่มต้น `gemini-3.6-flash`) พร้อม timeout, rate limit, ขนาดไฟล์สูงสุด และ structured-output validation
...
- ตั้งค่าที่ **ตั้งค่าระบบ > การชำระเงิน** (admin) — เลือกโหมด QR ร้านค้าถุงเงิน (อัปโหลดรูป QR จากแอปถุงเงิน; อ่านรูปใน browser และไม่เก็บรูปบน server) หรือพร้อมเพย์ส่วนตัว และ Slip2Go API Secret (เขียนอย่างเดียว เข้ารหัส AES-256-GCM ด้วย `APP_SECRET` ก่อนลงฐานข้อมูล client ไม่เห็น secret จริง)
...
อย่าตรวจคะแนนจาก `npm run dev` เพราะเป็น Vite development server ที่เปิด React Refresh
และส่ง source modules แบบไม่ optimize ให้หยุด dev server ก่อน แล้วใช้:

```bash
npm run audit:serve  # production build ที่ http://127.0.0.1:3000
```

จากนั้นเปิด Lighthouse ที่ `http://127.0.0.1:3000/`
