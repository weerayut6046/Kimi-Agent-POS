...
- หน้าจัดการกะรองรับการเลือก JPG/PNG/WebP หลายภาพหรือถ่ายเพิ่มทีละภาพ ระบบย่อภาพและลบ metadata ฝั่งเครื่องก่อนส่งไป backend
- Gemini Vision อ่านเลขตู้ โหมด `L` (ลิตร) / `P` (บาท) และจอซ้าย/ขวา โดยต่อเลขหลักนำหน้าด้านบนกับเลขบรรทัดล่างเป็น string ก่อนแปลงเป็นตัวเลข เช่น `L 42` + `0105.81` = `420105.81`
- ระบบจับคู่หัวจ่ายอัตโนมัติเมื่อชื่อหัวจ่ายระบุซ้าย/ขวา หากจับคู่ไม่ได้ต้องเลือกเอง ทุกค่าต้องผ่านหน้าตรวจทานก่อนเติมฟอร์ม และจะไม่ปิดกะอัตโนมัติ
- รูปถูกใช้เฉพาะคำขออ่านค่าปัจจุบันและไม่ได้บันทึกลงฐานข้อมูล; เมื่อบริการอ่านภาพใช้ไม่ได้ ผู้ใช้ยังกรอกมิเตอร์ด้วยตนเองได้ตามเดิม
- ตั้งค่า secret ฝั่ง backend/Edge เท่านั้น: `GEMINI_API_KEY`; ตัวเลือกเพิ่มเติมคือ `GEMINI_METER_OCR_MODEL` (ค่าเริ่มต้น `gemini-3.5-flash-lite`), `GEMINI_API_URL` และ `METER_OCR_TIMEOUT_MS` ห้ามใช้ชื่อขึ้นต้น `VITE_`
