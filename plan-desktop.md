...
Electron BrowserWindow
        │ HTTPS
        ▼
Vercel Frontend ── /api/* ──▶ Supabase Edge Functions + tRPC
                                      │ Drizzle + postgres.js
                                      ▼
...
- **Supabase Edge Functions**: ตรวจ Supabase Auth JWT และถือ server secrets; browser/Desktop ไม่เห็น service role, database URL หรือ `APP_SECRET`
...
| หลายจุดขาย | LAN ไปเครื่องหลัก          | HTTPS ไป Vercel/Supabase ชุดเดียวกัน              |
| Session    | header ที่ client กำหนดได้ | Supabase Auth access/refresh token                 |
| Backup     | ไฟล์ SQLite ในเครื่อง      | Supabase Managed Backups                           |
...
- ใช้ Supabase Managed Backups; restore ทำผ่าน Supabase Dashboard โดยผู้ดูแลโครงการ
...
- [x] **D7 — Multi-station**: รุ่น 2.0.0 แทน LAN server ด้วยระบบกลาง และปัจจุบันใช้ Vercel/Supabase พร้อม Supabase Auth
...
- [x] **D21 — Supabase central database (`2.0.0`)**: migrate schema/data เดิมครบ 25 ตารางและเปลี่ยน Desktop เป็น online shell; รุ่นปัจจุบันใช้ Supabase Edge Functions/Auth
...
| Supabase/Vercel ขัดข้อง                   | ตรวจ status/log, หยุด write traffic เมื่อจำเป็น และใช้แผน backup/PITR                                |
